...
func Test_bookingService_CreateNewBooking(t *testing.T) {
	...
	tests := []struct {
...

		{
			...
			args: args{&BookingRequest{Booking_id: "AB100620240002", User_id: "USID00002" , Ticket_name: "MARATHON THUNDER DAY", Booking_seat: "B03", Seat_id: 3, Amount: 1}},
...
func Test_bookingService_GetAllBookingByUser(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			fields: fields{repository.NewBookRepositoryDb(db), repository.NewSeatTicketRepositoryDb(db)},
			args: args{"USID00001"},
			want: []BookingResponse{{Ticket_name: "MARATHON THUNDER DAY", Booking_seat: "2", Seat_id: 0},{Ticket_name: "MARATHON THUNDER DAY", Booking_seat: "B02", Seat_id: 2}},
			wantErr: false,
		},
		// {
